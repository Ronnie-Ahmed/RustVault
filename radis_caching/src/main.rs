...
async fn main() {
    ...
    .route("/product/{id}", get(get_product_by_id).put(update_product).delete(delete_rpoduct)).with_state(db);
...
async fn test() -> Result<String, String> {
 ...
 async fn cache_set<T:Serialize>(
    ...
    let json=serde_json::to_string(value).expect("Seralization failed");
...
async fn cache_get<T: for<'a> Deserialize<'a>>(
    ...
    match result {
        Some(json) => {
            let data: T = serde_json::from_str(&json).expect("Deserialization failed");
            Ok(Some(data))
        }
        None => Ok(None),
    }
