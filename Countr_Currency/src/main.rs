...
async fn country_handler(Query(params): Query<Countryparams>)->Result<Json<CountryCurrency>,ApiError>{
...
async fn get_currency_rate(country: &str, ) -> Result<CountryCurrency, String> {
    ...
    let country_response:Value = client
        ...
        .await
        .map_err(|e| e.to_string())?;
    let currency_code=country_response["data"]["objects"][0]["currencies"][0]["code"].as_str().ok_or("Could not find country code".to_string())?.to_string();
    ...
    let currency_symbol = country_response["data"]["objects"][0]["currencies"][0]["symbol"]
        ...
        .unwrap_or("")
        .to_string();
    let country_name=country_response["data"]["objects"][0]["names"]["official"].as_str().ok_or("Could not find name".to_string())?.to_string();
    // Ok(CountryCode { country_name, currency_code, currency_symbol })
    let currency_url=format!("https://api.frankfurter.dev/v1/latest?base={}&symbols=USD",currency_code);
    let currency_response:CountryCurrency=client.get(&currency_url).send().await.map_err(|e| e.to_string())?.json().await.map_err(|e| e.to_string())?;
    ...
    // };
    Ok(CountryCurrency { amount: currency_response.amount, base: currency_response.base, date: currency_response.date, rates: currency_response.rates })
}
